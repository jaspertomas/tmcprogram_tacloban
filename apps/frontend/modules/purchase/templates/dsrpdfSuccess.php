...
// Multicell test
$widths=array(40,30,120,20,20,20,20,20);
$height=5;
...
$pdf->MultiCell($widths[7], $height, $content[7], 1, 'R', 0, 1, '', '', true);
...
foreach(array(1,2,3,4,5) as $template_id)
{
  ...
  foreach($events as $event)
  {
    ...
    if($purchase->getTemplateId()==$template_id)
    {
      $content=array(
        ...
        $purchase->getInvno()." (".MyDateTime::frommysql($purchase->getDate())->toshortdate().")",
        ...
        ($event->getDetail("chequeamt") and $purchase->getStatus()!="Cancelled")?$event->getDetail("chequeamt"):" ",
  ...
  foreach($purchases as $purchase)if($purchase->getTemplateId()==$template_id)
  {
      ...
      $content=array(
       $purchase->getVendor(),//." ".$purchase->getVendorName(),
      $purchase->getInvno()?$purchase->getInvno():" ",
       $particularsstring,
