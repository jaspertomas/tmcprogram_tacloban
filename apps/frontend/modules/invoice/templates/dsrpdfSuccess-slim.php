...
$pdf = new TCPDF("L", PDF_UNIT, "GOVERNMENTLETTER", true, 'UTF-8', false);
...
$widths=array(30,20,10,30,20,10,30,20,10,30,20,);
$height=1;
...
// Multicell test
$widths=array(30,18,20,70,18,18,18,18,18,18);
$height=5;
...
foreach(array(2,4,1,3) as $template_id)
{
  ...
  $pdf->MultiCell(246, 0, InvoiceTemplateTable::fetch($template_id), 1, 'L', 0, 1, '', '', true);
  ...
  foreach($events as $event)
  {
    ...
    if($invoice and $invoice->getTemplateId()==$template_id)
    {
      ...
      $pdf->MultiCell($widths[8], $height, $content[8], 1, 'C', 0, 0, '', '', true,0,true);
      $pdf->MultiCell($widths[9], $height, $content[9], 1, 'C', 0, 1, '', '', true,0,true);
    }
  }
...
*/
      ...
      ($invoice->getCredit()!=0 and $invoice->getStatus()!="Cancelled")?$invoice->getCredit():" ",
        $invoice->getDiscamt()==0?"":$invoice->getDiscamt(),
       $invoice->getEmployee()?$invoice->getEmployee():" ",
